            ...
            {viewApplicationsPageData.map((app, index) => (
              <tr key={index} className="text-gray-700">
                ...
                <td className="py-2 px-4 border-b text-center flex items-center">
                  <img className="w-10 h-10 rounded-full aspect-square mr-3 max-sm:hidden" src={app.imgSrc} alt="" />
                  <span>{app.name}</span>
                </td>
                <td className="py-2 px-4 border-b max-sm:hidden">{app.jobTitle}</td>
                <td className="py-2 px-4 border-b max-sm:hidden">{app.location}</td>
